...
class TestPolicyRunnerFromPretrained:
    ...
    def test_loads_remote_repo_with_revision(
        ...
        runner = PolicyRunner.from_pretrained("test/repo", revision="abc123")

        assert runner.device == "cuda"
        mock_act.ACTPolicy.from_pretrained.assert_called_once_with("test/repo", revision="abc123")
        mock_act.ACTPolicy.from_pretrained.return_value.to.assert_called_once_with("cuda")
        assert mock_pipeline.PolicyProcessorPipeline.from_pretrained.call_args_list[0].kwargs["revision"] == "abc123"
