...
| `RMW_IMPLEMENTATION`    | `rmw_fastrtps_cpp`                   | DDS implementation. Keep aligned with whatever publishes the camera.                                              |
...
| GET    | `/api/dataset/{id}/info`                      | Curated metadata summary.                                                |
| GET    | `/api/dataset/{id}/raw`                       | Full `info.json`.                                                        |
| GET    | `/api/dataset/{id}/thumbnail`                 | 320-pixel JPEG of frame 0.                                               |
| GET    | `/api/dataset/{id}/video`                     | Camera MP4 (HTTP `Range` supported, used by the in-grid previews).       |
| GET    | `/api/reference?dataset=<id>`                 | Full-resolution JPEG of the reference frame.                             |
| GET    | `/api/state`                                  | Current blend parameters.                                                |
| POST   | `/api/state`                                  | Update `dataset`, `ref_op`, `live_op`, `invert_ref`, `invert_live`.      |
...
Behavioural tests live under `tests/`. They cover the path-traversal guard, the LRU cache, the Pydantic state validator, the alignment-score low-texture floor, and the parquet episode reader. Run them inside the same image to inherit `rclpy`/`pyarrow`/`opencv`:
...
### Alignment score
The score is the zero-mean normalised cross-correlation (ZNCC) of the Sobel gradient magnitudes of the live and reference frames, in `[0, 1]`. Subtracting the mean removes additive brightness offsets; dividing by std removes multiplicative gain — so turning a lamp on/off, opening curtains, or tweaking exposure barely moves the gauge, but moved props do. A low-texture floor (variance threshold) returns `null` instead of a misleading 0/0 → 0 collapse for near-flat scenes; the UI surfaces that as "low texture: scene needs more detail to score". The displayed value is EMA-smoothed (α ≈ 0.4) and the gauge auto-zooms to the last ~12 s sliding window so fine adjustments stay visible.
