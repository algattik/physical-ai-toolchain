...
| `security/Test-HveCoreFreshness.ps1`  | Check pinned hve-core RPI ref and hve-core-derived files against the latest upstream release  |
...
The `Test-HveCoreFreshness.ps1` script is invoked by the `check-hve-core-freshness.yml` workflow on a weekly schedule. For both the RPI bootstrap persona files (umbrella agent and each discovered subagent) and every hve-core-derived module, it compares the **upstream** blob SHA at the pinned `UPSTREAM_REF` against the newest non-draft `microsoft/hve-core` release. Comparing blobs, not release tags, means a stale signal is a real upstream change rather than an unrelated prerelease.
