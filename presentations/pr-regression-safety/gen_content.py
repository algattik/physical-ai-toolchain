...
SLIDES = [
    ...
    {"kind": "code", "accent": GREEN, "title": "Recommendation \u2014 auto-merge, scoped tight",
     "caption": "`--auto` merges only AFTER the smoke gate is green \u2014 patch-only, dev/docs/actions, no runtime/GPU, no security",
    ...
    {"kind": "deflist", "accent": AMBER, "title": "Anticipated objections",
     ...
     "notes": "The objections a skeptical maintainer will raise, answered. Auto-merge is safe because it is scoped to patches in non-runtime areas with an instant revert. Replacing Dependabot now is premature when grouping is native and Renovate is a minority tool here. GPU on every pull request is too costly and unsafe for forks. And the prototype, run under emulation, is enough to prove the interpreter class, though a native runner would harden the final confidence."},
